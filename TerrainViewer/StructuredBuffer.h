...
namespace DirectX
{
    ...
    public:
        ...
        StructuredBuffer(ID3D11Device* device, const T* buffer, UINT size, CD3D11_BUFFER_DESC desc =
                             CD3D11_BUFFER_DESC(0, D3D11_BIND_SHADER_RESOURCE, D3D11_USAGE_DYNAMIC, D3D11_CPU_ACCESS_WRITE, D3D11_RESOURCE_MISC_BUFFER_STRUCTURED, sizeof(T)));
    ...
    template <typename T>
    StructuredBuffer<T>::StructuredBuffer(ID3D11Device* device, const T* buffer, const UINT size, CD3D11_BUFFER_DESC desc) :
        m_Capacity(size)
    {
    ...
    template <typename T>
    void StructuredBuffer<T>::SetData(ID3D11DeviceContext* context, const T* buffer, const UINT size)
    {

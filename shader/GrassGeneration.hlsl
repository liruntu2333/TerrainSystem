...
static const float PiDiv2 = 1.57079632679;
#define MaxBladeInTriangle 64
...
void main(uint3 threadId : SV_DispatchThreadID, uint3 groupId : SV_GroupID, uint3 groupThreadId : SV_GroupThreadID)
{
    ...
    const uint tri = groupId.x + groupId.y * groupCntX;

    if (tri >= numBaseTriangle)
    {
        return;
    }

    const BaseVertex baseV0 = vertices[indices[tri * 3 + 0]];
    const BaseVertex baseV1 = vertices[indices[tri * 3 + 1]];
    const BaseVertex baseV2 = vertices[indices[tri * 3 + 2]];
    ...
    const uint numSample = max(0, round(numBlade * lerp(1.0 - numRange, 1.0 + numRange, RandF(seed))));

    if (groupThreadId.x >= numSample)
    {
        return;
    }
    ...
    seed = threadId.x + threadId.y * groupCntX * MaxBladeInTriangle;
    ...
    {
        uint cullLevel = 500u;
        if (seed % cullLevel >= uint(ceil(max(1.0 - (camDist - 0.0) * 0.002, 0.0) * cullLevel)))
        {
            return;
        }
    }
    ...
    if (camDist < lod0Dist)
    {
        ...
        const float lod        = max((camDist - (1.0 - transition) * lod0Dist) / (transition * lod0Dist), 0.0);
        instance.lod           = lod;
